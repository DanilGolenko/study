...
OSRecord* writeDataToFile(const char* filename, OSRecord* records, int numRecords) {
...
int saveDataToFile(const char* filename, OSRecord* records, int numRecords) {
    FILE* file = fopen(filename, "wb");
    if (file == NULL) {
        printf("Ошибка при открытии файла для записи.\n");
        return 1;
...
OSRecord* searchRecordsByField(OSRecord* records, int numRecords) {
...
int sortRecords(OSRecord* records, int numRecords, const char* sortByField) {
...
int modifyRecord(OSRecord* records, int numRecords, int index) {
